...
\section{Computational model} \label{sec:computationalModel}
  ...
  \subsection{Load definition} \label{subsec:load_computationalModel}
    ...
    The computational model allowed different possibilities in terms of the load introduction. It was decided to locate the load introduction points on the upper flange of the ribs. The reason for this is that this will be the manner how the load will be introduced in a future manufactured demonstrator of the wing-box.

    Therefore, the number of load introduction points had a upper bound equal to the number of ribs available. For the baseline configuration, this number equals to three, one close rib at the tip of the wing-box and two open ribs in the inner part of the wing-box. Another possibility was to vary the position in the chordwise direction of load introduction point or points. In Figure \ref{fig:loadIntroductionPoints} it can be seen the location of the load introduction points when they are distributed among the three available ribs and their position in the chordwise direction is equal to $z / = 0.8 $

    \begin{figure}[!htpb]
      \centering
      \includegraphics[width=0.8 \textwidth]{../figures/model/loadIntroductionPoints}
      \caption[]{}
