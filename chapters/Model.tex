\chapter{Wing-box model description} \label{chap:Model}
...
\section{Computational model} \label{sec:computationalModel}
  ...
  \subsection{Lattice nodes rigid body modeling} \label{subsec:latticeNodesRigid_Parametrization}
    ...
    \begin{figure}[!htpb]
      \centering
      \includegraphics[width=0.8 \textwidth]{figures/model/closeLookToLatticeNodes}
  ...
  \subsection{Connection between the wing-box skin and the chiral lattice} \label{subsec:connections_computationalModel}
    ...
    % \begin{figure}[!htpb]
    %   \centering
    %   \includegraphics[width=0.6 \textwidth]{figures/model/connection-SYS1}
    ...
    % \begin{figure}[!htpb]
    %   \centering
    %   \includegraphics[width=0.7 \textwidth]{figures/model/connection-SYS2}
  ...
  \subsection{Post-processing operations} \label{subsec:postProc_computationalModel} %To be extended, maybe
    ...
    % PATHS, not important
    % Through the definition of paths in ABAQUS$^{\textregistered}$, it is possible to obtain the value of a determined magnitude for all the mesh elements located along the path. In Figure \ref{fig:pathUpper}, an example of a path located on the upper skin of the wing-box is shown.
    %
    % \begin{figure}[!htpb]
    %   \centering
